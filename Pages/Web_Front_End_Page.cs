...
namespace Translate_Management_System_Test.Pages
{
    class Web_Front_End_Page
    {
        ...
        public void Status_edited_control(string search_word)
        {
            ...
            var new_word = _driver.FindElement(By.XPath("//*[@id='translate0']/td/div/div[1]/div/div[1]/p")).Text;
            var old_word = _driver.FindElement(By.XPath("//*[@id='translate0']/td/div/div[2]/div/div[1]/p")).Text;
            Console.WriteLine(old_word + " is waiting to be converted to " + new_word);
        }
        ...
        public void return_approved_word(string search_word, string writed_word)
        {
            Search(search_word, "approved");
            btnAction.Click();
            var final = txtWrite.Text;
            if (writed_word == final)
            {
                Console.WriteLine("Written word :" + writed_word + ", The last status :" + final + " is equal.");
            }
            else
            {
                Console.WriteLine("Written word :" + writed_word + ", The last status :" + final);
                Console.WriteLine("Written word and last status is not equal.");
            }
